...
def variance_threshold_selection(
    ...
    kept = [c for c, v in zip(X.columns, selector.get_support(), strict=False) if v]
    return X[kept]


def mutual_information_selection(
    X: pd.DataFrame, y: pd.Series, k: int = 20, problem_type: str = "regression"
) -> pd.DataFrame:
    mi_func = (
        mutual_info_classif
...
def get_feature_importance(
    ...
    if hasattr(model, "feature_importances_"):
        scores = dict(zip(feature_names, model.feature_importances_, strict=False))
    elif hasattr(model, "coef_"):
        coefs = model.coef_.flatten() if model.coef_.ndim > 1 else model.coef_
        scores = dict(zip(feature_names, np.abs(coefs), strict=False))
    else:
        return {}
    sorted_scores = dict(
